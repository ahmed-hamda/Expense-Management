...
namespace ExpenseManagementSystemsVBDotNet
{
    public partial class UCExpenseForm : UserControl
    {
        ...
        {
            InitializeComponent();
            this.HandleCreated += UCExpenseForm_HandleCreated;

            DisplayCategory();
            displayExpenseList();
        }
        private void UCExpenseForm_HandleCreated(object? sender, EventArgs e)
        {
                ...
                else
                {
                    using (SqlConnection con = new SqlConnection(cs))
                    {
                        ...
                        string InsertData = @"
                                    ...
                                    (user_id, category, item, expense, description, date_expense, date_insert)
                                    VALUES
                                    (@uid, @category, @item, @expense, @description, @date_expense, GETDATE())";
